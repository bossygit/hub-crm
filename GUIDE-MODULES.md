...
**Automatisation** : les deux routes acceptent soit une session manager (ceo/manager/admin), soit `Authorization: Bearer $CRON_SECRET` pour les tâches planifiées. Définir `CRON_SECRET` dans les variables d'environnement Vercel pour activer les crons ; sinon utiliser les boutons manuels. Les relances email nécessitent `RESEND_API_KEY` **et un domaine d'envoi vérifié dans Resend** : l'expéditeur est configurable via `RESEND_FROM` (défaut `HUB-Distribution <contact@hub-distribution.com>` ; pour tester sans domaine validé, `RESEND_FROM=onboarding@resend.dev`, qui n'envoie qu'à l'adresse du compte Resend). Les échecs sont journalisés (console) et tracés dans `invoice_reminders.error` pour les relances.
